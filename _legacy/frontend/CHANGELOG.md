...
- v2バックエンドとの連携
  - バックエンドの一部をv2と連携しました。
    - 基本的な操作についてはv2で実施できるようになったため、大部分のバックエンドの置換を行いました。
    - OGP生成工程については、アーキテクチャ自体が異なるため残置しました。OGP生成自体は引き続き対応しますが、v2リリース後にv1のOGP生成機能は停止、またはv2準拠に置換する考えです。
    - 共有用ページの生成については、v2バックエンドの内容を踏襲したため制限がなくなりました。一方、これまで作成されたページの有効期限の変更はありません。

## 1.6.2

### Patch Changes

- バックエンドのデータベースのリクエスト制限により、一部投稿内容が表示できなくなる可能性に備え、エラーハンドリングに微修正を行いました。
  - クラウドサービスのRate Limit上限に達した場合のエラーを別途表示するように変更しました。
  - 一部コードのリファクタを実施しました。
