...
describe('RedisClient endpoint selection', () => {
    ...
    it('distribution approximates weights over many samples', async () => {
        ...
        for (const idx of [0, 1, 2]) {
            ...
            const relErr =
                Math.abs(obs - expected[idx]) / Math.max(expected[idx], 1e-9);
            ...
            balancing: [
                {
                    index: 0,
                    weight: 10,
                    registerBlock: { dateBefore: future.toISOString() },
                },
                { index: 1, weight: 1 },
                { index: 2, weight: 1 },
            ],
            ...
            balancing: [
                {
                    index: 0,
                    weight: 1,
                    migration: { dateAfter: past.toISOString(), weight: 20 },
                },
                { index: 1, weight: 1 },
                { index: 2, weight: 1 },
            ],
            ...
            balancing: [
                {
                    index: 0,
                    weight: 1,
                    registerBlock: { dateAfter: past.toISOString() },
                },
                { index: 1, weight: 1 },
            ],
        };
            ...
            balancing: [
                {
                    index: 0,
                    weight: 1,
                    migration: { dateBefore: future.toISOString(), weight: 20 },
                },
                { index: 1, weight: 1 },
                { index: 2, weight: 1 },
            ],
