    ...
    for (const name of candidates) {
        ...
        if (fs.existsSync(p)) {
            try {
                ...
                console.warn(
                    `[set-balancing-env.cjs] failed to parse ${p}: ${e && e.message}`,
                );
            }
        }
    }
...
if (balancing) {
    console.info(
        '[set-balancing-env.cjs] DB_ENDPOINT_RULE loaded from balancing file',
    );
} else {
    console.info(
        '[set-balancing-env.cjs] no balancing file found, leaving DB_ENDPOINT_RULE as-is',
    );
}
...
if (args.length === 0) {
    console.error(
        'Usage: node set-balancing-env.cjs [--write-env <file>] -- <command>',
    );
...
if (cmdArgs[0] === '--write-env' && cmdArgs[1]) {
    ...
    if (!balancing) {
        console.info(
            '[set-balancing-env.cjs] no balancing file found; nothing to write',
        );
        process.exit(0);
    }
    ...
    try {
        ...
        console.info(
            `[set-balancing-env.cjs] wrote DB_ENDPOINT_RULE to ${envPath}`,
        );
        process.exit(0);
    } catch (e) {
        console.warn(
            `[set-balancing-env.cjs] failed to write env file ${envFileCandidate}: ${e && e.message}`,
        );
        process.exit(1);
    }
}
